"""Runnable local-tool incident triage demo for ThreadSwarm."""
...
DEFAULT_INPUT_RESOURCE = files("src.demos").joinpath("data/incident_bundle.txt")
...
def incident_tool(
    ...
    if tool_name == "extract-error-signatures":
        ...
        ranked = [{"signature": signature, "count": count} for signature, count in counts.most_common()]
        return {
            "instruction": instruction,
...
def load_bundle_text(input_file: Path | None = None) -> str:
    source = input_file or DEFAULT_INPUT_RESOURCE
